...
def detect_recency(content: str, url: str) -> tuple:
    ...
    if not detected_years:
        stale_penalty: int = int(RECENCY_CONFIG['stale_penalty'])
        return (stale_penalty, None)

    most_recent_year = int(max(detected_years))
    current_year = int(RECENCY_CONFIG['current_year'])
    ...
    if year_diff == 0:
        ...
        # Two years old
        recency_score = 5
    elif most_recent_year in [int(y) for y in RECENCY_CONFIG['acceptable_years']]:
        # Within acceptable range
        recency_score = 5
    else:
        # Stale content
        recency_score = int(RECENCY_CONFIG['stale_penalty'])
